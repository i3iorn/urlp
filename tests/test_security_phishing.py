...
class TestPhishingDBBasicFunctionality:
    ...
    def test_check_against_phishing_db_detects_known_host(self):
        ...
        with patch("urlps._security.request.urlopen", return_value=mock_resp) as mocked_get:
    ...
    def test_check_against_phishing_db_returns_false_for_safe_host(self):
        ...
        with patch("urlps._security.request.urlopen", return_value=mock_resp):
    ...
    def test_caching_prevents_multiple_downloads(self):
        ...
        with patch("urlps._security._download_phishing_db", return_value={"one", "two"}) as mocked_get:
...
class TestPhishingDBNetworkFailures:
    ...
    def test_handles_generic_network_error(self):
        ...
        with patch("urlps._security.request.urlopen", side_effect=OSError("network")) as mocked_get:
    ...
    def test_handles_url_error(self):
        ...
        with patch("urlps._security.request.urlopen", side_effect=URLError("DNS lookup failed")):
    ...
    def test_handles_http_404_error(self):
        ...
        with patch("urlps._security.request.urlopen", side_effect=error):
            assert _security.check_against_phishing_db("any.host") is False
    ...
    def test_handles_connection_timeout(self):
        ...
        with patch("urlps._security.request.urlopen", side_effect=socket.timeout("timed out")):
            assert _security.check_against_phishing_db("any.host") is False

    def test_handles_connection_refused(self):
        """Connection refused should result in empty set."""
        with patch("urlps._security.request.urlopen", side_effect=ConnectionRefusedError()):
            assert _security.check_against_phishing_db("any.host") is False
    ...
    def test_handles_ssl_error(self):
        ...
        with patch("urlps._security.request.urlopen", side_effect=ssl_error):
            assert _security.check_against_phishing_db("any.host") is False
...
class TestDownloadPhishingDBDirectly:
    ...
    def test_returns_empty_set_on_failure(self):
        ...
        with patch("urlps._security.request.urlopen", side_effect=OSError("fail")):
            result = _security._download_phishing_db()
            assert result == set()
...
class TestPhishingDBRefresh:
    ...
    def test_refresh_phishing_db_redownloads(self):
        ...
        with patch("urlps._security.request.urlopen", return_value=mock_resp1):
        ...
        with patch("urlps._security.request.urlopen", return_value=mock_resp2):

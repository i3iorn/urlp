# AGENTS.md — Guide for AI coding agents working on urlp
...
- `urlp/__init__.py` — canonical public surface (exports: `parse_url`, `parse_url_unsafe`, `build`, `URL`).
- `urlp/url.py` — main `URL` class (immutable), audit hooks, and convenience helpers.

Architecture (big picture)
- Parser → Components → Builder → URL facade:
  - `urlp/_parser.py` (stateless parsing + `Parser` backward-compat wrapper).
  - `urlp/_components.py` (immutable ParseResult / URLComponents dataclasses).
  - `urlp/_builder.py` (percent-encoding, path normalization, query serialization).
  - `urlp/url.py` (immutable `URL` object: `with_*` methods, canonicalize, semantic equality).
- Validation vs Security:
  - `urlp/_validation.py` — pure validation helpers (no network I/O, many cached functions).
  - `urlp/_security.py` — security-sensitive checks (SSRF, DNS rebinding, phishing DB, path traversal).
- Exceptions: `urlp/exceptions.py` defines the hierarchy; prefer catching specific types (e.g., `HostValidationError`) when appropriate.
...
- Audit hooks: `set_audit_callback`, `get_audit_callback` (used in `urlp/_audit.py`, invoked during parse).
...
- Caches exist (`lru_cache`) for validators and security checks — if you change canonicalization or validation logic, clear caches in tests (`Validator.clear_caches()`, `urlp._security.clear_caches()`).
...
- `urlp/_parser.py` — path normalization, host/port parsing, query parsing rules.
- `urlp/_builder.py` — encoding, serialize_query, add/remove/merge param behaviors (duplicate keys preserved order).
- `urlp/_validation.py` — regex-based validators and IDNA handling; cache sizes/hits matter for perf.
- `urlp/_security.py` — SSRF logic, decimal/octal/hex ip parsing, phishing DB download. Treat tests referencing these as high-sensitivity.
...
- For security-related changes: include rationale and re-run `tests/test_security_*` and `bandit -r urlp`.
